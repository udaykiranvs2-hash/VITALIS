...
function LandingPage() {
  const pricingPlans = [
    { name: 'Free', price: '₹0', description: 'Get started with essential symptom checks, report summaries, and doctor discovery.', features: ['Basic symptom checks', 'Limited report summaries', 'Access to doctor directory'], cta: 'Start free', link: '/register' },
    { name: 'Plus', price: '₹699 / mo', description: 'Unlock unlimited insights, advanced analysis, and faster specialist matching.', features: ['Unlimited symptom checks', 'Full report analysis', 'Priority doctor matches'], cta: 'Get Plus', link: '/register' },
    { name: 'Pro', price: '₹2,499 / mo', description: 'Designed for families and care teams with collaborative tools and dedicated support.', features: ['Team accounts & sharing', 'Advanced analytics', 'Dedicated support'], cta: 'Get Pro', link: '/register' }
  ];
  ...
  return (
    <div className="landing-page">
      ...
      <section className="landing-pricing" id="pricing">
        <div className="landing-section-head"><p className="landing-section-pretitle">Pricing</p><h2 className="landing-section-title">Simple plans for individuals and care teams</h2><p className="landing-section-subtitle">Choose a plan that fits your needs — start free and upgrade as you scale.</p></div>
        <div className="pricing-grid landing-pricing-grid">{pricingPlans.map((plan) => (<article key={plan.name} className="pricing-card"><h3>{plan.name}</h3><p className="price">{plan.price}</p><p className="pricing-description">{plan.description}</p><ul>{plan.features.map((feature) => <li key={feature}>{feature}</li>)}</ul><Link to={plan.link} className="landing-cta-button">{plan.cta}</Link></article>))}</div>
      </section>
